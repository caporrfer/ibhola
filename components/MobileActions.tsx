"use client";

import { useEffect, useState } from "react";
import { Navigation, Phone, Star } from "lucide-react";
import { business } from "@/config/business";

export function MobileActions() {
  const [visible, setVisible] = useState(false);

  useEffect(() => {
    const updateVisibility = () => {
      const hero = document.querySelector<HTMLElement>(".hero, .page-hero, .cs-hero");
      const revealPoint = window.innerHeight * 0.25;
      setVisible(!hero || hero.getBoundingClientRect().bottom <= revealPoint);
    };

    updateVisibility();
    window.addEventListener("scroll", updateVisibility, { passive: true });
    window.addEventListener("resize", updateVisibility);
    return () => {
      window.removeEventListener("scroll", updateVisibility);
      window.removeEventListener("resize", updateVisibility);
    };
  }, []);

  return <div className={`mobile-action-bar ${visible ? "is-visible" : ""}`} aria-label="Acciones rápidas" aria-hidden={!visible}>
    <a href={`tel:${business.phone}`} aria-label={`Llamar a IBHOLA: ${business.phoneDisplay}`} title="Llamar a IBHOLA" tabIndex={visible ? undefined : -1}><Phone size={24} aria-hidden="true" /></a>
    <a href={business.mapsUrl} target="_blank" rel="noreferrer" aria-label="Ver cómo llegar a IBHOLA" title="Cómo llegar" tabIndex={visible ? undefined : -1}><Navigation size={24} aria-hidden="true" /></a>
    <a href={business.googleReviewsUrl} target="_blank" rel="noreferrer" aria-label="Escribir una reseña de IBHOLA en Google" title="Valóranos en Google" tabIndex={visible ? undefined : -1}><Star size={24} aria-hidden="true" /></a>
  </div>;
}
