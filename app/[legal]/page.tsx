...
export default async function LegalPage({ params }: { params: Promise<{ legal: string }> }) {
  ...
  return (
    <main className="legal-page">
      ...
      <article><p className="eyebrow"><span /> Información legal</p><h1>{doc.title}</h1><p>{doc.text}</p><hr /><h2>Datos de contacto publicados</h2><p>{business.name}<br />{fullAddress}<br />Tel. {business.phoneDisplay}</p><small>Última revisión: agosto de 2026.</small></article>
    </main>
  );
}
